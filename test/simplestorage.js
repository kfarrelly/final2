...
contract('ERC721g', async (accounts) => {
  ...
  let isOnline = await instance.isOnline.call();
  let backOn = await instance.toggleOnlineStatus({from: alice})
 let isOnline1 = await instance.isOnline.call();

  assert.equal(owner, accounts[0], "Account 1 does not match owner of contract") 
  assert.equal(total, 0, "The value totalGenerated was not 0");
  assert.equal(isOnline, false, "The contract is online when it should be")
  assert.equal(isOnline1, true, "The contract is not online when it should be")
  ...
  it("it should create a new erc721", async () => {
    let  instance = await ERC721g.deployed();
  let newToken = await instance.createERC721(name, url, url, input3, input2, input1, number, {from: alice});
  ...
  it("it should check the address of the current tokens, add the address and confirm its name", async () => {
    ...
    assert.equal(newContract.address, address, "The two address values are the same")
   ...
   it("should transfer the erc721 correctly", async () => {
    ...
    let attempt = await newContract.transferFrom(alice, bob, 1, {from: alice})
